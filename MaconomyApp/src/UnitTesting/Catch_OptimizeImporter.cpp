...
TEST_CASE("Testing import of time log from html file.", "[OptimizeImport]") {
	const std::string path{ "src/UnitTesting/Catch_OptimizeImporter_" };
	...
	Importer::ptr importer = std::make_unique<OptimizeImporter>(config.get());
	refSet set{};

	SECTION("Importing simple time log") {
		config->timeLogPath = path + "timeLog1.html";
	...
	SECTION("Importing full time log complex") {
		config->timeLogPath = path + "timeLog2.html";
		std::vector<Entry*> res = importGetEntries(importer.get());
